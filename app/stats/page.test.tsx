...
describe("StatsPage — DC-aware states wording", () => {
  it("labels the states stat 'State covered' with a DC footnote, shows the state count (1) not the raw jurisdiction total (2), and phrases the top-states caption '1 state and DC covered' when the dataset includes DC", async () => {
    mockGetStats.mockReset().mockResolvedValue({
      count: 5,
      states: 2,
      includesDc: true,
    ...
    // `${stats.states} state${...}` template, fails one of these.
    expect(screen.getByText("State covered")).toBeInTheDocument();
    ...
    // (1), not the raw jurisdiction total (2) — "2 / States covered" over a
    // footnote naming DC reads as "two states, plus DC." Reverting the call
    // site to pass the raw total back to `value` fails this.
    const statesTile = screen.getByText("State covered").closest("div");
    expect(statesTile).not.toBeNull();
    expect(within(statesTile!).getByText("1")).toBeInTheDocument();
    // The singular caption is only honest because the footnote carries DC.
