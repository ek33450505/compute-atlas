...
export function ContestedStrip({
  ...
  return (
    <section aria-labelledby="contested-heading" className={className}>
      {/* `defer-offscreen` (globals.css) skips this subtree's layout and paint
          until it is near the viewport; `--defer-h` comes from the caller and
          inherits down to here. Read that utility's comment before touching
          `p-2 -m-2` — the padding is load-bearing, not spacing. In short:
          `content-visibility: auto` turns on paint containment unconditionally,
          which clips descendants to THIS element's padding box, and the case
          grid below is edge-flush with cards whose focus ring paints 4px
          outside their border box. The 8px reserve keeps that ring (and the
          `.plate-hover` tilt) inside the clip; the equal negative margin hands
          the space back, so nothing moves.

          It sits on a wrapper rather than on the <section> because the section
          carries the caller's full-width `border-t`, which a negative margin
          would drag into the page gutter. */}
      <div className="defer-offscreen p-2 -m-2">
